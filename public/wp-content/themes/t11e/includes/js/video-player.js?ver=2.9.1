$(window).bind('load', function() {
    ...
    $(".screencast[rel]").overlay({
        // use the Apple effect for overlay
        effect: 'apple',
        expose: '#333',
        onLoad: function(content) {
            // find the player contained inside this overlay and load it
            if (!$.browser.msie && Modernizr.video.h264 !== "") {
                $(function(){
                    VideoJS.setup();
                });
            } else {
        ...
        onClose: function(content) {
            ...
            for (var i=0; i<videoJSPlayers.length; i++) {
                videoJSPlayers[i].video.pause();
                videoJSPlayers[i].video.currentTime = 0;
            }
        }
    });
    ...
    if (!$.browser.msie && Modernizr.video.h264 !== "") {
        ...
        $(".video-js-box").remove();
        $("a.player").flowplayer("/wp-content/themes/t11e/includes/swf/flowplayer-3.1.5.swf");
    }
